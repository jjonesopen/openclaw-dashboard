...
// ── OpenClaw Gateway check (API-based) ───────────────────────
async function checkGateway() {
  const health = await fetchJSON(`${config.gatewayUrl}/health`);
  const active = health?.ok === true;
  return { name: 'openclaw-gateway', active, uptime: null };
}
...
export async function getServiceStatus() {
  const [ollama, gateway, gog, github] = await Promise.all([
    ...
    checkGateway(), // Use API check instead of systemctl for user service
    checkGog(),
    checkGitHub(),
  ]);
